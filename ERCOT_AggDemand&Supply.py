...
def parse_ercot_curve(df, mw_keyword, price_keyword, is_cumulative=True):
    ...
    for mw_col, price_col in zip(mw_cols, price_cols):
        if mw_col in df.columns and price_col in df.columns:
            ...
            # Use cumulative math for SCED, but NOT for DAM
...
def plot_dam():
    ...
    # FIX: Turned is_cumulative OFF for all DAM graphs!
    dam_demand = parse_ercot_curve(bids_filtered, mw_keyword='Bid MW', price_keyword='Price', is_cumulative=False)
    
    eo_offers_df = pd.read_csv(FILE_DAM_OFFERS)
    eo_filtered = clean_and_filter(eo_offers_df, TARGET_DATE, TARGET_HOUR)
    dam_supply_eo = parse_ercot_curve(eo_filtered, mw_keyword='Offer MW', price_keyword='Price', is_cumulative=False)
